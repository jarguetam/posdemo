...
export class PrintOupPutService {
    constructor(private commonService: CommonService) {}

    async printRequestOutPut(output: InventoryOutPutModel) {
        const doc = new jsPDF({
            orientation: 'p',
            unit: 'pt',
            format: 'letter',
        ...
        let pages: number = parseInt(
            (output.detail.length / itemPerPage).toString()
        );
        ...
        for (let index = 0; index < pages + pageExtra; index++) {
            ...
            let items = detail
                .slice(itemPerPage * index, output.detail.length)
                .slice(0, itemPerPage);
            ...
            doc.text('Salida de mercancia', 300, 40);
            ...
            //doc.text("Contenedor: " +output.outputId, 20, 125);
            doc.text('Comentario: ' + output.comment, 20, 140);
            doc.text("Tipo: " +output.typeName, 20, 120);
            // doc.text("Consolidador: " +output.consolidator, 175, 140);
            doc.text('Fecha y Hora:', 570, 140);
            doc.text(new Date(output.outputDate).toLocaleString(), 650, 140);

            doc.setFont('helvetica', 'normal', 'bold');
            doc.setFontSize(14);
            doc.text('N°: ' + output.outputId, 570, 40);
            doc.setFontSize(12);
