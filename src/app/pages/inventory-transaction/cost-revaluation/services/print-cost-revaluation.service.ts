...
import { CostRevaluationModule } from '../cost-revaluation.module';

@Injectable({
  providedIn: 'root'
})

export class PrintCostRevaluationService {

    constructor(private commonService: CommonService) {
    }


  async printRequestEntry(entry: CostRevaluationModel) {
     const doc = new jsPDF({
       orientation: "p",
       unit: "pt",
       format: 'letter'
     ...
     let pages: number = parseInt((entry.detail.length / itemPerPage).toString());
     let pagesMod: number = parseInt((entry.detail.length % itemPerPage).toString());
     ...
     for (let index = 0; index < pages + pageExtra; index++)
     {
         ...
         let items = detail.slice((itemPerPage * index), entry.detail.length).slice(0, itemPerPage);

                 //Primer Columna
         doc.setFontSize(14);
         doc.setFont('helvetica','normal', 'bold')
         doc.text(this.commonService.companyInfo().companyName, 20, 40);
         doc.text("Revalorizacion de inventario", 300, 40);
         ...
         doc.text("Correo: "+ this.commonService.companyInfo().email1 +"/ "+ this.commonService.companyInfo().email2, 20, 94);
         //Segunda Columna
       ...
       //  doc.addImage("/assets/img/pos-terminal.png", "JPEG", 660, 20, 80, 60);
         // doc.setFontSize(14);
         doc.setFont('helvetica','normal', 'bold')
         //doc.text("Detalle del Ingreso:", 20, 270);
         //doc.text("Contenedor: " +entry.entryId, 20, 125);
         doc.text("Comentario: " +entry.comment, 20, 140);
        // doc.text("Consolidador: " +entry.consolidator, 175, 140);
         doc.text("Fecha y Hora:", 570, 140);
         doc.text(new Date(entry.createDate).toLocaleString(), 650, 140);

         doc.setFont('helvetica','normal', 'bold')
         doc.setFontSize(14);
         doc.text("N°: "+entry.id, 570, 40);
         doc.setFontSize(12);

         if (items.length > 0)
         {
             this.generateDetailDiff(doc, items, alturaDefecto, (index + 1 >=  pages + pageExtra), entry.whsName);
         }
     ...
     private async  generateDetailDiff(doc: jsPDF,detail: CostRevaluationModel[], alturaDefecto: number, isLastPage: boolean, whsName: string){
        let totalQty =  0;
        let totalWeight =  0;
        ...
        doc.text(`Almacen`, 270, alturaDefecto +7 );


        doc.text(`Costo Anterior`, 410, alturaDefecto+ 7);
        doc.text(`Costo Actual`, 500, alturaDefecto+ 7);
         ...
         for (var indexItem = 0; indexItem < (detail??[]).length; indexItem++)
        {
            ...
            doc.text(`${item.previousCost.toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })}`, 410, lastHeight);
            doc.text(`${item.newCost.toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })}`, 500, lastHeight);
            // doc.text(`${new Date(item.dueDate).toLocaleDateString('es-HN')}`, 600, lastHeight);
            // doc.text(`${item.lineTotal.toLocaleString('es-HN', { style: 'currency', currency: 'HNL' })}`,700 , lastHeight);
      ...
      //  doc.rect(20, lastHeight+15, 750, 0.7, "F");
        ...
        if(isLastPage){
            ...
            doc.text(``+totalWeight.toLocaleString('es-HN', { style: 'currency', currency: 'HNL' }), 700, lastHeight + 35 );
        }
