...
export class PrintEntryService {
    constructor(private commonService: CommonService) {}

    async printRequestEntry(entry: InventoryEntryModel) {
        const doc = new jsPDF({
            orientation: 'p',
            unit: 'pt',
            format: 'letter',
        ...
        let pages: number = parseInt(
            (entry.detail.length / itemPerPage).toString()
        );
        ...
        for (let index = 0; index < pages + pageExtra; index++) {
            ...
            let items = detail
                .slice(itemPerPage * index, entry.detail.length)
                .slice(0, itemPerPage);
            ...
            doc.text('Entrada de mercancia', 300, 40);
            ...
            doc.text('Tipo: ' + entry.typeName, 20, 120);
            // doc.text("Consolidador: " +entry.consolidator, 175, 140);
            doc.text('Fecha y Hora:', 570, 140);
            doc.text(new Date(entry.entryDate).toLocaleString(), 650, 140);

            doc.setFont('helvetica', 'normal', 'bold');
            doc.setFontSize(14);
            doc.text('N°: ' + entry.entryId, 570, 40);
            doc.setFontSize(12);
