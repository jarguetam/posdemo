...
export class PrintInvoiceSaleService {
    ...
    private generateHeader(doc: jsPDF, document: DocumentSaleModel) {
        ...
        doc.setFont('helvetica', 'normal', 'bold');
        doc.text('Factura N°: ' + document.deiNumber, 290, 120);
        ...
        // Cliente Titulos
        doc.setFont('helvetica', 'normal', 'bold');
        doc.text(`Datos Generales del cliente`, 20, 140);

        //Columna derecha
        doc.setFont('helvetica', 'normal', 400);

        doc.setFont('helvetica', 'normal', 'bold');
        doc.text('Fecha de factura:', 290, 140);
        doc.text('Fecha de vencimiento:', 460, 140);
        doc.setFont('helvetica', 'normal', 400);
        doc.text(
        ...
        doc.setFont('helvetica', 'normal', 'bold');
        doc.text(document.customerCode + ' ' + document.customerName, 20, 157);
        doc.setFont('helvetica', 'normal', 400);
        doc.text(`R.T.N.: ` + document.customerRTN, 20, 175);
        doc.text(`Direccion: ` + document.customerAddress, 20, 185);
        ...
        // Columna Derecha sar
        doc.setFont('helvetica', 'normal', 'bold');
        doc.text('Régimen de facturación autorizado por la SAR', 290, 170);
        doc.rect(280, 160, 300, 0.7, 'F');
        doc.text(`CAI: ` + document.cai, 290, 190);
        doc.rect(280, 180, 300, 0.7, 'F'); //Linea abajo
        doc.text(
        ...
        doc.rect(280, 200, 300, 0.7, 'F'); //Linea abajo
        doc.text(
    ...
    private async generateDetail(
        ...
        if (isLastPage) {
            ...
            doc.text(document.sellerName, 30, lastHeight + 400);
            doc.rect(30, lastHeight + 405, 120, 0.7, 'F');
            doc.text(`Elaborado por`, 30, lastHeight + 420);
        }
