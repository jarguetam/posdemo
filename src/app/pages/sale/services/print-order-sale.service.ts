...
export class PrintOrderSaleService {
    ...
    private generateHeader(doc: jsPDF, document: DocumentSaleModel) {
       ...
       // doc.addImage('./assets/img/pos-terminal.png', 'JPEG', 470, 40, 80, 60);

        doc.setFont('helvetica', 'normal', 'bold');
        doc.text('Orden de venta N°: ' + document.docId, 300, 120);
    ...
    async printOrder(document: DocumentSaleModel) {
        const doc = new jsPDF({
            orientation: 'p',
            unit: 'pt',
            format: 'letter',
