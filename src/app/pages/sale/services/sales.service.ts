...
export class SalesService {
    ...
    async getInvoiceActiveCustomer(idCustomer: number) {
        try {
            let invoice = await firstValueFrom(
                this.http.get<DocumentSaleModel[]>(
            ...
            let data = await this.db.invoiceSeller
                .where('customerId')
                .equals(idCustomer)
                .toArray();
            return data;
    ...
    async getInvoiceActiveSeller(idSeller: number) {
        try {
            ...
            this.db.invoiceSeller.clear();
            invoices.map((inv) => this.db.invoiceSeller.add(inv));
            return invoices;
        } catch (error) {
            let invoicesOffline = await this.db.invoiceSeller.toArray();
    ...
    async addInvoice(entry: DocumentSaleModel) {
        try {
            const apiDataPromise = firstValueFrom(
                this.http.post<DocumentSaleModel[]>(`${environment.uriLogistic}/api/Sales/AddInvoiceSale`, entry).pipe(
                    catchError((error) => throwError(() => error)),
                    timeout(10000) // Espera máximo 5 segundos para la respuesta de la API
                )
            );
